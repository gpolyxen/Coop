...
void AWeaponBase::MulticastFireEffects_Implementation(FVector_NetQuantize Muzzle,FRotator Rotation){if(MuzzleFlash&&Mesh)UNiagaraFunctionLibrary::SpawnSystemAttached(MuzzleFlash,Mesh,TEXT("b_gun_muzzleflash"),FVector::ZeroVector,FRotator::ZeroRotator,EAttachLocation::SnapToTarget,true,true,ENCPoolMethod::None,true);if(FireSound)UGameplayStatics::PlaySoundAtLocation(this,FireSound,Muzzle);if(ACharacter*C=Cast<ACharacter>(GetOwner()))if(CharacterFireMontage&&C->GetMesh()&&C->GetMesh()->GetAnimInstance())C->GetMesh()->GetAnimInstance()->Montage_Play(CharacterFireMontage);}
