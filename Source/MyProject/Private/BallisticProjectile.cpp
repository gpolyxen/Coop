...
void ABallisticProjectile::InitializeProjectile(float D,float Dr,float W,float GravityScale,float LifeSeconds,AController* I)
{
	Damage=D;
	Drag=Dr;
	WindInfluence=W;
...
void ABallisticProjectile::OnProjectileHit(UPrimitiveComponent*,AActor* Other,UPrimitiveComponent*,FVector,const FHitResult& Hit)
{
	...
	if(Cast<AZombieCharacter>(Other))
		UGameplayStatics::ApplyPointDamage(Other,Damage,Movement->Velocity.GetSafeNormal(),Hit,DamageInstigator.Get(),this,nullptr);
	...
	else UGameplayStatics::ApplyPointDamage(Other,Damage,Movement->Velocity.GetSafeNormal(),Hit,DamageInstigator.Get(),this,nullptr);
	Destroy();
}
