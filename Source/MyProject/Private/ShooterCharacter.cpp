...
AShooterCharacter::AShooterCharacter()
{
	...
	Inventory=CreateDefaultSubobject<UInventoryComponent>(TEXT("Inventory"));Health=CreateDefaultSubobject<UHealthArmorComponent>(TEXT("Health"));
...
void AShooterCharacter::StartAim(){bIsAiming=true;}void AShooterCharacter::StopAim(){bIsAiming=false;}
void AShooterCharacter::StartFire(){if(!EquippedWeapon)return;FireOnce();const float Interval=60.f/FMath::Max(1.f,EquippedWeapon->Stats.RoundsPerMinute);GetWorldTimerManager().SetTimer(FireTimer,this,&AShooterCharacter::FireOnce,Interval,true,Interval);}void AShooterCharacter::StopFire(){GetWorldTimerManager().ClearTimer(FireTimer);}void AShooterCharacter::FireOnce(){if(!EquippedWeapon)return;RecoilPitchTarget=FMath::Clamp(RecoilPitchTarget+EquippedWeapon->RecoilPitch,0.f,4.f);RecoilYawTarget=FMath::Clamp(RecoilYawTarget+FMath::FRandRange(-EquippedWeapon->RecoilYaw,EquippedWeapon->RecoilYaw),-1.5f,1.5f);RecoilKickTarget=FMath::Clamp(RecoilKickTarget+EquippedWeapon->RecoilKickback,0.f,5.f);const FVector ViewStart=Camera->GetComponentLocation(),ViewEnd=ViewStart+Camera->GetForwardVector()*100000.f;FHitResult Hit;FCollisionQueryParams Params(TEXT("WeaponAim"),true,this);Params.AddIgnoredActor(EquippedWeapon);const bool bHit=GetWorld()->LineTraceSingleByChannel(Hit,ViewStart,ViewEnd,ECC_Visibility,Params);const FVector AimPoint=bHit?Hit.ImpactPoint:ViewEnd;EquippedWeapon->Fire((AimPoint-EquippedWeapon->GetMuzzleLocation()).GetSafeNormal());}void AShooterCharacter::ResetFirstPersonArmsAnimation(){if(FirstPersonIdleAnimation)FirstPersonArms->PlayAnimation(FirstPersonIdleAnimation,true);}void AShooterCharacter::Reload(){StopFire();if(EquippedWeapon)EquippedWeapon->Reload();}void AShooterCharacter::SprintPressed(){GetCharacterMovement()->MaxWalkSpeed=SprintSpeed;}void AShooterCharacter::SprintReleased(){GetCharacterMovement()->MaxWalkSpeed=WalkSpeed;}
...
void AShooterCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>&OutLifetimeProps)const{Super::GetLifetimeReplicatedProps(OutLifetimeProps);DOREPLIFETIME(AShooterCharacter,EquippedWeapon);DOREPLIFETIME(AShooterCharacter,WeaponSlots);DOREPLIFETIME(AShooterCharacter,ActiveWeaponSlot);}
